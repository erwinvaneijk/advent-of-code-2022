...
isVisibleFromLeft row col m = V.all (< h) (rowToLeft row col m)
  where
    h = MA.getElem row col m

isVisibleFromRight :: Int -> Int -> MA.Matrix Int -> Bool
isVisibleFromRight row col m = V.all (< h) (rowToRight row col m)
  where
    h = MA.getElem row col m
...
isVisible row _ m
    | MA.nrows m == row = True
isVisible row col m =
    isVisibleFromLeft row col m ||
    isVisibleFromRight row col m ||
    isVisibleFromTop row col m || isVisibleFromBottom row col m

visibleInts :: MA.Matrix Int -> [Bool]
visibleInts m =
    [isVisible r c m | r <- [1 .. (MA.nrows m)], c <- [1 .. (MA.ncols m)]]
...
treesLowerToLeft row col m =
    treesLower (height row col m) $ V.reverse (rowToLeft row col m)

treesLowerToRight :: Int -> Int -> MA.Matrix Int -> Int
treesLowerToRight row col m =
    treesLower (height row col m) (rowToRight row col m)

treesLowerToTop :: Int -> Int -> MA.Matrix Int -> Int
treesLowerToTop row col m =
    treesLower (height row col m) $ V.reverse (colToTop row col m)

treesLowerToBottom :: Int -> Int -> MA.Matrix Int -> Int
treesLowerToBottom row col m =
    treesLower (height row col m) (colToBottom row col m)

scenicScore :: Int -> Int -> MA.Matrix Int -> Int
scenicScore row col m =
    treesLowerToLeft row col m * treesLowerToRight row col m *
    treesLowerToTop row col m *
    treesLowerToBottom row col m
...
maxScenicScore m =
    maximum $
    [scenicScore r c m | r <- [1 .. (MA.nrows m)], c <- [1 .. (MA.ncols m)]]

day08a :: MA.Matrix Int :~> Int
day08a = MkSol {sParse = Just . parse, sShow = show, sSolve = Just . numVisible}

day08b :: _ :~> _
day08b =
    MkSol {sParse = Just . parse, sShow = show, sSolve = Just . maxScenicScore}
